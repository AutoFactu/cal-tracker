...
export type AgentRunResult =
  ...
  | { kind: "meal_corrected"; meal?: Meal; proposal?: MealProposal; message: string }
  ...
  | { kind: "food_memory"; matches: unknown[]; message: string; options?: unknown[] }
  | { kind: "nutrition_search"; items: MealItem[]; message: string; options?: unknown[] }
...
export class AgentService {
  ...
  async run(text: string, context: ActionContext): Promise<AgentRunResult> {
    ...
    try {
      ...
      if (fallbackToolCall) {
        const result = await this.actionExecutor.execute(fallbackToolCall.function.name, JSON.parse(fallbackToolCall.function.arguments), {
          ...context,
      ...
      return {
        ...
        message: "The agent provider is unavailable. Please rephrase or try again.",
      };
    }
    ...
    switch (actionId) {
      case "query_food_memory": {
        ...
        return {
          ...
          matches,
          options: matches,
          message: matches.length > 0 ? "I found matching food memories." : "I couldn't find matching food memories.",
        };
      }
      case "search_nutrition_database": {
        ...
        return {
          ...
          items,
          options: items,
          message: items.length > 0 ? "I found matching nutrition items." : "I couldn't find matching nutrition items.",
        };
      }
      ...
      case "commit_meal":
        return { kind: "meal_committed", meal: output.meal as Meal, message: "Meal logged." };
      case "correct_meal": {
        const meal = output.meal as Meal | undefined;
        const proposal = output.proposal as MealProposal | undefined;
        if (meal) return { kind: "meal_corrected", meal, message: "Meal corrected." };
        return { kind: "meal_corrected", proposal, message: "Meal proposal corrected." };
      }
      ...
      case "get_usual_meals":
        return { kind: "templates", templates: output.templates as MealTemplate[], message: "Here are your usual meals." };
      case "create_meal_template":
        return { kind: "template_saved", template: output.template as MealTemplate, message: "Usual meal created." };
      case "update_meal_template":
        return { kind: "template_saved", template: output.template as MealTemplate, message: "Usual meal updated." };
      case "delete_meal_template":
        return { kind: "template_deleted", deleted: Boolean(output.deleted), message: output.deleted ? "Usual meal deleted." : "Usual meal was not found." };
      case "delete_meal": {
        const confirmationRequired = (output as { confirmationRequired?: boolean }).confirmationRequired;
        if (confirmationRequired) {
          return { kind: "confirmation_required", actionId: "delete_meal", input: { ...output, originalText }, message: "Please confirm deletion." };
        }
...
function isMealLoggingIntent(text: string): boolean {
  ...
  if (/^(how|cuanto|cuanta|cuantas|cuantos|que|what)\b/.test(normalized)) return false;
  if (/\b(delete|remove|borrar|eliminar|corrige|correct|corregir)\b/.test(normalized)) return false;
  return /\b(log|add|ate|had|consumed|record|registrar|registro|anade|anadir|agrega|agregar|apunta|apuntar|comi|comido|tome|consumi|desayuno|almuerzo|comida|cena|merienda|snack)\b/.test(normalized);
}
...
function fallbackToolCallForText(text: string): AgentToolCall | null {
  ...
  if (/\b(remaining|left|quedan|restan|calorias restantes|calories left)\b/.test(normalized)) {
    return toolCall("get_remaining_targets", {});
  }
  if (/\b(summary|resumen|today|hoy)\b/.test(normalized)) return toolCall("get_daily_summary", {});
  if (/\b(history|historial|ultimas comidas|recent meals)\b/.test(normalized)) return toolCall("get_meal_history", { limit: 10 });
  if (/\b(usual meals|templates|plantillas|comidas habituales|habituales)\b/.test(normalized)) return toolCall("get_usual_meals", {});

  const nutritionSearch = /\b(?:search|buscar|lookup|consulta|consultar)\b.*\b(?:nutrition|nutricion|food|alimento|database|base)\b(?:\s+(?:for|de|para))?\s*(.*)$/.exec(normalized);
  if (nutritionSearch?.[1]) return toolCall("search_nutrition_database", { query: nutritionSearch[1].trim() });
