...
void main() {
  patrolTest('edits a Home meal card with explicit item details', ($) async {
    final user = await _createPatrolUser();
    final mealId = await _seedCommittedMeal(user.accessToken);
    ...
    await $('Patrol breakfast').waitUntilVisible(
      timeout: const Duration(seconds: 20),
    );
    expect($(find.byIcon(Icons.add_rounded)), findsNothing);

    await $(ValueKey('dashboard_edit_meal_$mealId')).tap();
    expect($(const ValueKey('dashboard_item_name_0')), findsOneWidget);
    expect($(const ValueKey('meal_correction_field')), findsNothing);

    await $(const ValueKey('dashboard_item_quantity_0')).enterText('120');
    await $(const ValueKey('dashboard_item_calories_0')).enterText('300');
    await $(const ValueKey('save_dashboard_item_edits_button'))
        .scrollTo()
        .tap();
...
Future<String> _seedCommittedMeal(String accessToken) async {
  ...
  final body = jsonDecode(response.body) as Map<String, Object?>;
  final output = body['output'] as Map<String, Object?>;
  final meal = output['meal'] as Map<String, Object?>;
