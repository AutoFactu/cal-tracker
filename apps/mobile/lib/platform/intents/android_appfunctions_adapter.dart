...
class AndroidAppFunctionsAdapter {
  const AndroidAppFunctionsAdapter(
      {required PlatformActionBridge bridge, this.enabled = false})
      : _bridge = bridge;
  ...
  Future<Map<String, Object?>> getDailySummary() {
    if (!enabled) {
      return Future.value({'authenticationRequired': false, 'enabled': false});
    }
